...
class ConnectionMonitorTask:
    ...
    def _init_monitor_task(self):
        ...
        async def before_monitor():
            ...
            # 等待客户端初始化并连接成功
            while not self.client_manager or not hasattr(self.client_manager, 'client') or not self.client_manager.is_connected:
                await asyncio.sleep(2)
                if self.client_manager and hasattr(self.client_manager, 'is_shutting_down') and self.client_manager.is_shutting_down:
